class ApplicationModel
{
    ...
    createAccount(user, name, password, category)
    {
        ...
        this._saveUserToIndexedDB({
            user, name, password, category, failedLoginCounter: 0, isLocked: false
        });
    ...
    EditUserAccount(accountName, newUser, newName, newPassword, newCategory)
    {
        ...
        this._saveUserToIndexedDB(userData);
    ...
    EliminateAccount(accountName)
    {
        ...
        this._authData.delete(accountName);
        this._deleteUserFromIndexedDB(accountName);
    ...
    _initIndexedDB() {
        ...
        request.onsuccess = (event) => {
            ...
            this._loadUsersFromIndexedDB(); // cargar usuarios al inicio
        };
    ...
    _saveUserToIndexedDB(user) {
        const transaction = this._db.transaction(["users"], "readwrite");
        const store = transaction.objectStore("users");
        store.put(user);
    }

    _deleteUserFromIndexedDB(userName) {
        const transaction = this._db.transaction(["users"], "readwrite");
        const store = transaction.objectStore("users");
        store.delete(userName);
    }

    _loadUsersFromIndexedDB() {
        ...
        request.onsuccess = () => {
            ...
            if (users.length > 0) {
                ...
                // Si está vacío, usar usuarios por defecto
                for (let user of this._users_Data) {
                    this._authData.set(user.name, { ...user, isLocked: false });
                    this._saveUserToIndexedDB({ ...user, isLocked: false });
                }
            }
        };
    }
...
| --------------------------- | --------------------------------- | ------------------------------------------- |
| Tipo de almacenamiento      | Clave-valor plano                 | Base de datos orientada a objetos           |
| Capacidad                   | Limitada (5-10MB)                 | Mucho mayor (hasta cientos de MBs)          |
| Tipos de datos              | Sólo cadenas                      | Objetos completos                           |
| Rendimiento en búsqueda     | Poco eficiente                    | Indexación por clave, mejor rendimiento     |
| Operaciones asincrónicas    | No                                | Sí (basado en eventos/callbacks)            |
| Persistencia entre pestañas | Sí (local), No (session)          | Sí                                          |
| Complejidad de uso          | Muy baja                          | Moderada (requiere callbacks/transacciones) |
