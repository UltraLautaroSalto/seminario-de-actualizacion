...
string Client_Name = "Lautaro";
string Client_password = "1234";

string Admin_Name = "Admin";
string Admin_password = "1999";

string Seller_Name = "Seller";
string Seller_password = "2077";

string WarehouseWorker_Name = "WarehouseWorker";
string WarehouseWorker_password = "1001";
...
void Buy_Productos(vector<Productos>& productos, int id){
    int Cantidad_Compra;
    for(auto it = productos.begin(); it != productos.end(); ++it){
        if(it->Id_Producto == id){
            cout << "Cuantas Unidades desea comprar el Producto Seleccionado?" << endl;
            cout << "Ingrese su Eleccion Aqui: ";
            cin >> Cantidad_Compra;

            it->Stock_Producto -= Cantidad_Compra;

            cout << "Stock Actual Del Producto: " << it->Stock_Producto << endl;
            Opciones_Productos();
        }
    }
}
...
void Opciones_Productos(){
    ...
    if(Opciones_Product == "1"){
        ...
        cout << endl << endl;
        ///////////////////////////////////////////////////////////////////////
        cout << "\n Ingrese el ID del Producto a Comprar: " << endl;
        cin >> id;
        Buy_Productos(productos, id);
...
void Identificacion(){
    ...
    string Ident_user_password;
    int trials = 0;

    while(trials < 3){
    cout << "Ingrese [Nombre_de_Usuario] y [Password]:" << endl;
    cin >> Ident_user_name;
    cin >> Ident_user_password;

        if (Ident_user_name == Admin_Name && Ident_user_password == Admin_password){
            cout << "Biendenido Administrador" << endl;
            User_Aceptado();
            break;
        } else if(Ident_user_name == Seller_Name && Ident_user_password == Seller_password){
            cout << "Biendenido Vendedor" << endl;
            User_Aceptado();
            break; 
        }else if(Ident_user_name == WarehouseWorker_Name && Ident_user_password == WarehouseWorker_password){
            cout << "Biendenido Trabajador de Deposito" << endl;
            User_Aceptado();
            break; 
        }else if(Ident_user_name == Client_Name && Ident_user_password == Client_password){
            cout << "Bienvenido Cliente " << Client_Name << endl;
            User_Aceptado();
            break;
        }else if(Ident_user_name != Client_Name || Ident_user_password != Client_Name){  // "||" es para decir "O una o la otra"
...
void CrearUserName(){
    ...
    if (ValidarUserName(NewUser_Name)) {
        ...
        Client_Name = NewUser_Name;  // Actualiza la contraseña
...
void CrearUserPassword(){
    ...
    if (ValidarPassword(CreatePassword)) {
        ...
        Client_password = CreatePassword;  // Actualiza la contraseña
...
void NuevaPassword(){
    ...
    if (ValidarPassword(NewPassword)) {
        cout << "La Password es Valida." << endl;
        Client_password = NewPassword;  // Actualiza la contraseña
        cout << "Password Cambiada Exitosamente" << endl;
        User_Aceptado();
    } else {
        cout << "Contraseña no válida. Debe tener entre 8 y 16 caracteres, "
